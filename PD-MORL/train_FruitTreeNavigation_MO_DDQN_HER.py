...
if __name__ == "__main__":
    ...
    env_main = gym.make(args.scenario_name, depth= args.depth)
    env_main.seed(args.seed)
    test_env_main = gym.make(args.scenario_name, depth= args.depth)
    ...
    x = np.array(FRUITS_DICT[str(args.depth)]) * np.power(args.gamma, args.depth-1)
    ...
    # interp = NearestNDInterpolator(w_batch_interp, x_unit)
    ...
    try:
        for ts in tqdm(range(0, PROCESSES_COUNT*args.time_steps,PROCESSES_COUNT)): #iterate through the fixed number of timesteps
            ...
            if sum(process_episode_array) > (PROCESSES_COUNT*args.eval_freq*eval_cnt):
                eval_cnt +=1
                CR_F1, hypervolume, sparsity, _ = lib.utilities.MORL_utils.eval_agent_discrete(test_env_main, agent_main, w_batch_test[np.random.randint(0,len(w_batch_test),size=500)], args)
                ...
                if max_metric <= model_eval_metric:
                    ...
                    lib.utilities.common_utils.save_model(net, args, name = 'FruitTreeNavigation_MO_DDQN_HER', ext ='{}_{}'.format(time_step,args.depth))
    finally:
    ...
    if max_metric <= model_eval_metric:
        max_metric = model_eval_metric
        max_metric_time_step = time_step
        lib.utilities.common_utils.save_model(net, args, name = 'FruitTreeNavigation_MO_DDQN_HER',ext ='{}_{}'.format(max_metric_time_step,args.depth))
